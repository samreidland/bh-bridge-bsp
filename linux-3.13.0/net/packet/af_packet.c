		       ...
		       struct packet_type *pt, struct net_device *orig_dev)
{
	...
	if (skb->pkt_type == PACKET_LOOPBACK || (sk->sk_filter_drop_match == 1 && skb->pkt_type == PACKET_OUTGOING))
		goto drop;
	...
	/* the packet passed the filter check, drop it unconditionally */
	ret = sk->sk_filter_drop_match;
